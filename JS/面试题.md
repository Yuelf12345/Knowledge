...
## 普通函数和箭头函数的区别
  1. this 的绑定
    普通函数：this 的值取决于函数的调用方式，是动态绑定的
    箭头函数：this 继承自外层作用域，是词法绑定的（lexically bound）
  2. arguments 对象
    普通函数：有内置的 arguments 对象，包含传递给函数的所有参数
    箭头函数：没有自己的 arguments 对象
  3. 构造函数
     普通函数：可以作为构造函数，使用 new 关键字创建实例
     箭头函数：不能作为构造函数使用，没有 [[Construct]] 方法
  4. 原型属性
    普通函数：具有 prototype 属性 fn.prototype是一个对象
    箭头函数：没有 prototype 属性

## typeof 和 instanceof 区别
    typeof 返回一个字符串，表示该对象的类型。
    instanceof 返回一个布尔值，表示该对象是否为该构造函数的实例。
```javascript
...
```
## new 一个箭头函数会怎么样
这段代码会抛出一个 TypeError(is not a constructor)错误。 箭头函数没有 this，所以 new 一个箭头函数会报错。
箭头函数与普通函数有几个重要区别：
- 没有构造函数：箭头函数不能用作构造函数，不能使用 new 关键字调用
- 没有 this 绑定：箭头函数不绑定自己的 this，它会捕获其所在上下文的 this 值
- 没有 arguments 对象：箭头函数内部没有 arguments 对象
- 不能用作生成器：箭头函数不能用 yield 关键字
  
## dom事件流
- 捕获阶段：事件从window对象开始向下传播，依次经过document、html、body等祖先元素，直到到达目标元素的父级元素
- 目标阶段：事件到达目标元素本身
- 冒泡阶段：事件从目标元素开始向上传播，依次经过父元素、祖先元素，直到window对象
 stopPropagation() 阻止事件继续传播（包括捕获和冒泡）
